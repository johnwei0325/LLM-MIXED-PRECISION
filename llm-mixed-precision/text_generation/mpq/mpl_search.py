...
def mpl(model_name, model_t, data, target_avg_bit=8):
    ...
    if 'gpt2' in model_name:
        for layer in model_t.transformer.h:
            eps_a_s.append(layer.eps_a.view(n, -1).float().data.cpu().numpy())
    ...
    # re-map
    omega = (-att_corr_distort - np.percentile(-att_corr_distort, 85)) / np.std(-att_corr_distort)
...
#with torch.autocast(cast_device):
    ...
    ## re-map
    omega = (att_corr_distort - np.percentile(att_corr_distort, 50)) / np.std(att_corr_distort)
