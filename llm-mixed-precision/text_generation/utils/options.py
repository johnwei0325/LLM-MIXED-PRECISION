...
MODEL = 'facebook/opt-125m' # 'facebook/opt-125m', 'gpt2', 'llama-7b'
METHOD = 'all' # 'wikitext', 'ptb', 'c4', 'lambada'
QMETHOD = 'mpl_plus' # 'rtn' ('uniform'), 'gptq', 'smoothquant', 'ptqvit'
...
## Warm-up 
parser.add_argument('--gpus', type = str, default = '0', help = 'Select gpu to use')
...
#parser.add_argument('--model_path', type = str, default = '/home/tachen/model', help = 'The directory where the input data is stored.')
parser.add_argument('--model_path', type = str, default = '/home/ta/research/model', help = 'The directory where the input data is stored.')
...
parser.add_argument('--job_dir', type = str, default = f'../../llm_experiment/{MODEL.split("/")[-1]}/{QMETHOD}/{INDEX}/', help = 'The directory where the summaries will be stored.') # 'experiments/'
parser.add_argument('--search_dir', type = str, default = f'../../llm_experiment/{MODEL.split("/")[-1]}/{QMETHOD}/s_0/', help = 'The directory where the summaries will be stored.') # 'experiments/'
...
parser.add_argument("--mgpus", default=0, type=int)
parser.add_argument("--port", default='29500', type=str)

parser.add_argument('--finetuned', type = str_to_bool, default = False, help = 'Whether to use a finetuned model')
parser.add_argument('--test_only', type = str_to_bool, default = False, help = 'Test or not')
parser.add_argument('--bit_search_only', type = str_to_bool, default = BIT_SEARCH_ONLY, help = 'Test or not')
...
parser.add_argument('--qmethod', type = str, default = QMETHOD, help = 'Load pruned model')
...
parser.add_argument( '--tasks',  type = str, default = 'all', help = 'Text generation of classification.')
parser.add_argument("--lam_perp", default=1, type=float, help="(log) perplexity regularizer")

## Training
parser.add_argument('--bitW', type = int, default = 16, help = 'Quantized bitwidth.') # None
...
parser.add_argument('--fix_bitW', type = str_to_bool, default = False, help = 'Fixed weight bitwidth.') # None
parser.add_argument('--fix_abitW', type = str_to_bool, default = False, help = 'Fixed activation bitwidth.') # None


parser.add_argument('--target_model', type = str, default = 'OPTClass', help = 'The target model.') # 'GPT2LMHeadModel', 'OPTClass', 'LlamaForCausalLM'
...
parser.add_argument('--eval_batch_size', type = int, default = 2, help = 'Batch size for validation.')
