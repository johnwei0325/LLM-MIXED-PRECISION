...
def main():
    ...
    if 'opt' in args.model:
        ...
        if args.model == 'llama-7b':
            ...
            state_dict = torch.load(os.path.join('/home/johnwei743251/llm-mixed-precision/model', 'pretrained_llama_7b.pt'))
            config = LLaMAConfig()
            ...
            #state_dict = model.state_dict()
            #torch.save(state_dict, '/home/johnwei743251/llm-mixed-precision/model/pretrained_llama_13b.pt')
            state_dict = torch.load(os.path.join('/home/johnwei743251/llm-mixed-precision/model', 'pretrained_llama_13b.pt'))
        ...
        if args.qmethod == 'rtn':
            ...
            model_file_name = f'model.llama_quant_mpl'
        
        model_t = import_module(model_file_name).__dict__[args.target_model](config)
        
        state_dict_t = dict()
        
        for k, v in model_t.model.state_dict().items():
            if k in state_dict:
                state_dict_t[k] = state_dict[k]
            else:
                state_dict_t[k] = model_t.model.state_dict()[k]
        ...
        device_map = infer_auto_device_map(
            model_t.model,
            max_memory=max_memory,  # Adjust for your GPUs
            no_split_module_classes=["LlamaDecoderLayer"]  # Prevent splitting critical layers
        )
        print("Device map:", device_map)
        model_path = '/home/johnwei743251/llm-mixed-precision/model'
        state_dict_file = 'pretrained_llama_7b.pt'
        state_dict_path = os.path.join(model_path, state_dict_file)
        model_t.model.load_state_dict(state_dict_t)
        
        print("Dispatching the model across GPUs...")
        model_t.model = load_checkpoint_and_dispatch(
            model_t.model, 
            state_dict_path,  # Point directly to the checkpoint file
            device_map=device_map, 
            offload_folder="offload",  # Use disk if GPU memory is insufficient
            offload_state_dict=True   # Offload large parts to disk if necessary
        )

        print("Model successfully loaded and distributed across GPUs!")
    ...
    if args.qmethod == 'ompq':
        ...
        with torch.autocast(cast_device):
            if 'llama' in args.model:
                ...
                model_t = model_t if len(gpus) > 1 else model_t.to(device)
            bitcfg, quant_err, omega = mpl_search.mpl(args.model, model_t, data.to(device), args.bitW)
    ...
    abitcfg = np.full(len(bitcfg), 4)

    avg_bit, org_model_size, quant_model_size = param_calculator(bitcfg)
    quant_bop = bop_calculator(bitcfg, abitcfg)
    ...
    print_logger.info(f"Quantization model BOPs: {quant_bop} (F)")
    ...
    #device_map = infer_auto_device_map(model_t.model, max_memory={f"cuda:{gpu}": "15GiB" for gpu in gpus}, no_split_module_classes=["LlamaDecoderLayer"])
    #print("Device map inferred:", device_map)
